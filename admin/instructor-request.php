...
if(isset($_GET['actionActive'])){
    if(isset($_SESSION['admin_login'])){
        ...
        $queryAction = "UPDATE instructor SET ins_status = 'active' WHERE ins_user_id = '$id' ";
  ...
  if(isset($_GET['actionDeactive'])){
    if(isset($_SESSION['admin_login'])){
        ...
        $queryAction = "UPDATE instructor SET ins_status = 'deactive' WHERE ins_user_id = '$id' ";
        $actionRow   = $db->update($queryAction);
  ...
  if(isset($_GET['actionDelete'])){
    if(isset($_SESSION['admin_login'])){
        ...
        $queryAction = "DELETE FROM instructor WHERE ins_user_id = '$id' ";
        $actionRow   = $db->update($queryAction);
...
<div class="grid_10">
    ...
    $query = "SELECT * FROM instructor ORDER BY ins_status DESC";
    ...
    if($read)
    {
     ...
     ?>
		...
		<tr>
            ...
            <th><?php echo $row['ins_user_id']; ?></th>
            <td><?php echo Format::formatDate($row['ins_registered']); ?></td>
            <td><?php
            if($row['ins_status'] == "active"){
                echo "<a class='text-danger' href='?actionDeactive={$row['ins_user_id']}'>Deactive</a>";
            }elseif($row['ins_status'] == "deactive"){
                echo "<a class='text-success' href='?actionActive={$row['ins_user_id']}'>Active</a>";
            }
                
             ?>|
             <a href=''>Edit</a>|
             <a href="?actionDelete=<?php echo $row['ins_user_id']; ?>">Delete</a>
             </td>
